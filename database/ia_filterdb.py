...
async def get_search_results(query, file_type=None, max_results=8, offset=0, filter=False):
    ...
    tasks = [Media2.find(filter_query).sort('$natural', -1).to_list(length=25), Media3.find(filter_query).sort('$natural', -1).to_list(length=25)]
    ...
    all_files =  files_media3 + files_media2
